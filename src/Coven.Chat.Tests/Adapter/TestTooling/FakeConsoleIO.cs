...
public sealed class FakeConsoleIO : IConsoleIO
{
    ...
    private readonly Channel<string> _outChannel = Channel.CreateUnbounded<string>(new UnboundedChannelOptions { SingleReader = false, SingleWriter = false });
    ...
    {
        _out.Enqueue(line);
        _ = _outChannel.Writer.TryWrite(line);
    ...
    public Task<string> ReadOutputAsync(CancellationToken ct = default)
        => _outChannel.Reader.ReadAsync(ct).AsTask();
}
