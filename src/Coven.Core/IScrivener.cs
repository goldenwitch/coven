...
public interface IScrivener<TJournalEntryType> where TJournalEntryType : notnull
{
    ...
    Task<long> WriteAsync(TJournalEntryType entry, CancellationToken cancellationToken = default);
    ...
    IAsyncEnumerable<(long journalPosition, TJournalEntryType entry)> TailAsync(long afterPosition = 0, CancellationToken cancellationToken = default);
    ...
    IAsyncEnumerable<(long journalPosition, TJournalEntryType entry)> ReadBackwardAsync(long beforePosition = long.MaxValue, CancellationToken cancellationToken = default);
    ...
    Task<(long journalPosition, TJournalEntryType entry)> WaitForAsync(long afterPosition, Func<TJournalEntryType, bool> match, CancellationToken cancellationToken = default);
    ...
    /// <param name="afterPosition">Only consider entries strictly after this position.</param>
    /// <param name="match">Predicate to select the desired derived entry.</param>
    /// <param name="cancellationToken">A cancellation token.</param>
    /// <returns>The first matching (journalPosition, entry) pair with the derived entry.</returns>
    Task<(long journalPosition, TDerived entry)> WaitForAsync<TDerived>(long afterPosition, Func<TDerived, bool> match, CancellationToken cancellationToken = default)
